...
```
So we can see that we are dealing with a 64-bit binary with a Stack Canary and Non-Executable Stack. When we run this binary, we can see that we are prompted for input(which we gave it `15935728`). It then provided us the date and time.
Decompile the binary file with Ghidra, we got:
```c
...
So we can see the program prints the string `Are you a big boiiiii??` with `puts`. Then it proceeds to scan in 0x18 bytes worth of data into `input`. In addition to that, we can see the target integer is initialized before the `read` call. Looking at the decompiled code shows us the constants it is assigned and compared to as signed integers, however if we look at the assembly code, we can see the constants as unsigned hex integer.

We can see the value is being assigned in `0xdeadbeef`
```asm
...
We can also see that the value that is being compared to is 0xcaf3baee:
```asm
        004006a5 8b 45 e4        MOV        EAX,dword ptr [RBP + target]
        004006a8 3d ee ba        CMP        EAX,0xcaf3baee
                 f3 ca
```
Now to see what our input can reach, we can look at the stack layout in Ghidra. To see this, we just double click on any variables where they are decleared:
```asm
                             **************************************************************
                             *                          FUNCTION                          *
                             **************************************************************
...
Here we can see that according to Ghidra, input is stored at the offset -0x38. 
The target is stored at the offset -0x24.
This mean that there is a 0x14 byte difference between two value.
Since we can write 0x18 bytes, that means we can filled up the 0x14 byte difference and overwrite 4 bytes of target. 
Here the bug is it is letting us write 0x18 bytes worth of data to a 0x14 byte space, and 0x4 bytes of data are overflowing into the target variable which gives us the ability to change what it is. Taking a look at the memory layout in gdb gives us a better description.
**We set a breakpoint for directly after the read call and see what the memory looks like:**
...
Here we can see that we have overwritten the integer with the value `0xcaf3baee`. when we continue onto the cmp instruction, we will pass the check.
```bash
gef➤  b *0x4006a8
...
With all of that, we can write the exploit for this challenge:
```py
#import pwntools
from pwn import *
